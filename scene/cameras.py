...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        if gt_alpha_mask is not None:
            gt_alpha_mask = torch.tensor(gt_alpha_mask).to(dtype=torch.float).to(self.data_device)
            self.gt_alpha_mask = gt_alpha_mask.reshape(-1, self.image_height, self.image_width)
